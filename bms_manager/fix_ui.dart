...
void main() {
  ...
  final startMarker = RegExp(r'  @override\s+Widget build\(BuildContext context\) \{\s+return Scaffold\(');
  final endMarker = RegExp(r'class BmsMetrics \{');

  final startMatch = startMarker.firstMatch(text);
  final endMatch = endMarker.firstMatch(text);

  if (startMatch != null && endMatch != null) {
    // only get the one inside _BmsDashboardPageState
    // find index after _isPollEcho
    final isPollEchoMatch = RegExp(r'bool _isPollEcho\(List<int> raw\) \{').firstMatch(text);
    if (isPollEchoMatch == null) { print("poll echo null"); return; }
    
    final realStartMatch = startMarker.allMatches(text).firstWhere((m) => m.start > isPollEchoMatch.end);
    
    final startIdx = realStartMatch.start - 12; // roughly to include '@override'

    final endIdx = endMatch.start;

    final newUi = '''  @override
...
''';
    final newText = text.substring(0, startIdx) + newUi + text.substring(endIdx);
