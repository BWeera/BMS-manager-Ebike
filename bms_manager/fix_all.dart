...
void main() {
  ...
  // Fix 1: ValueNotifier
  text = text.replaceFirst('void main() {', 'final ValueNotifier<ThemeMode> themeNotifier = ValueNotifier(ThemeMode.system);\n\nvoid main() {');

  // Fix 2: App Theme
  final appStart = text.indexOf('return MaterialApp(');
  final appEnd = text.indexOf('    );\n  }\n}\n', appStart) + 11;
  text = text.replaceRange(appStart, appEnd, '''return ValueListenableBuilder<ThemeMode>(
  ...
  // Fix 3: UI Replacement
  var startMarker = '  @override\r\n  Widget build(BuildContext context) {\r\n    return Scaffold(';
  var startIdx = text.indexOf(startMarker);
  if (startIdx == -1) startIdx = text.indexOf('  @override\n  Widget build(BuildContext context) {\n    return Scaffold(');
  
  var endMarker = 'class BmsMetrics {\r\n';
  var endIdx = text.indexOf(endMarker);
  if (endIdx == -1) endIdx = text.indexOf('class BmsMetrics {\n');
...
''';
  ...
  text = text.substring(0, startIdx) + newUi + text.substring(endIdx);
